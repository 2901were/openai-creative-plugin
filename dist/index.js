...
var require_ucs2length = __commonJS({
  "node_modules/ajv/lib/compile/ucs2length.js"(exports, module) {
    "use strict";
    module.exports = function ucs2length(str2) {
...
var Protocol = class {
  ...
  async notification(notification, options) {
    ...
    const debouncedMethods = (_b = (_a3 = this._options) === null || _a3 === void 0 ? void 0 : _a3.debouncedNotificationMethods) !== null && _b !== void 0 ? _b : [];
  ...
  constructor(_serverInfo, options) {
    ...
    if (this._capabilities.logging) {
      this.setRequestHandler(SetLevelRequestSchema, async (request, extra) => {
        var _a4;
        const transportSessionId = extra.sessionId || ((_a4 = extra.requestInfo) === null || _a4 === void 0 ? void 0 : _a4.headers["mcp-session-id"]) || void 0;
...
var ConfigSchema = external_exports.object({
  openaiApiKey: external_exports.string().min(1, "OpenAI API key is required")
});
var MAX_SESSIONS = 5;
...
var MODEL_REGISTRY = {
  "gpt-image-2": {
    ...
    // Not supported as of 2026-06-05 (verified: API rejects background:"transparent").
    // OpenAI may re-add — flip this flag and the types.test.ts assertion together.
    supportsTransparentBackground: false,
    supportsArbitrarySize: true,
    sizeOptions: ["auto", "1024x1024", "1536x1024", "1024x1536"],
    supportsInputFidelity: false,
    // always-high fidelity; the param is rejected — omit it
    supportsEdits: true
    // pending Task 0 empirical confirmation
  },
  "gpt-image-1.5": {
    id: "gpt-image-1.5",
...
var ConfigManager = class {
  ...
  async saveConfig() {
    if (this.config) {
      const configPath = path2.join(process.cwd(), ".openai-creative-config.json");
...
async function handleConfigureOpenAIToken(request, configMgr) {
...
async function handleGetConfigurationStatus(configMgr) {
  ...
  if (isConfigured) {
    ...
    switch (configMgr.configSource) {
      case "environment":
        sourceInfo = "\n\u{1F4CD} Source: Environment variable (OPENAI_API_KEY)\n\u{1F4A1} This is the most secure configuration method.";
        break;
      case "config_file":
        sourceInfo = "\n\u{1F4CD} Source: Local configuration file (.openai-creative-config.json)\n\u{1F4A1} Consider using environment variables for better security.";
        break;
    }
  } else {
...
\u{1F4DA} Configuration options (in priority order):
1. \u{1F947} MCP client environment variables (Recommended)
2. \u{1F948} System environment variable: OPENAI_API_KEY
3. \u{1F949} Use configure_openai_token tool

\u{1F4A1} For the most secure setup, add this to your MCP configuration:
...
function validateOutputDirectory(dirPath) {
  if (!path3.isAbsolute(dirPath)) {
    throw new McpError(
      ...
      `Output directory must be an absolute path. Received: ${dirPath}`
    );
  }
  const tmpDir = os.tmpdir();
  if (dirPath === tmpDir || dirPath.startsWith(tmpDir + path3.sep)) {
    const resolved2 = path3.resolve(dirPath);
    if (resolved2 !== dirPath) {
      console.warn(`[validateOutputDirectory] Path normalized: ${dirPath} -> ${resolved2}`);
    }
    return resolved2;
  }
  const dangerous = [
  ...
  for (const dangerousDir of dangerous) {
    if (normalizedPath === dangerousDir.toLowerCase() || normalizedPath.startsWith(dangerousDir.toLowerCase() + path3.sep)) {
      throw new McpError(
        ErrorCode.InvalidParams,
        `Cannot use system directory as output: ${dirPath}`
      );
    }
  }
...
function getImagesDirectory(session) {
  ...
  if (platform === "win32") {
    return path3.join(homeDir, "Documents", "openai-creative-images");
  } else {
    return path3.join(homeDir, "openai-creative-images");
  }
}
function getModelCapabilities(modelId) {
  ...
  if (!capabilities) {
    ...
    throw new McpError(
      ErrorCode.InvalidParams,
      `Unknown model: ${modelId}. Available models: ${availableModels}`
    );
  }
  return capabilities;
}
...
var ARBITRARY_TARGET_PIXELS = 1048576;
var round16 = (n) => Math.max(16, Math.round(n / 16) * 16);
function resolveImageSize(modelId, opts) {
  const caps = getModelCapabilities(modelId);
  if (opts.size) {
    if (opts.size === "auto" || caps.sizeOptions.includes(opts.size)) return opts.size;
    if (!caps.supportsArbitrarySize) {
      throw new McpError(
        ErrorCode.InvalidParams,
        `Model ${caps.displayName} supports only preset sizes: ${caps.sizeOptions.join(", ")}. For arbitrary WxH sizes use gpt-image-2.`
      );
    }
    const m = SIZE_RE.exec(opts.size);
    const w2 = m ? Number(m[1]) : NaN;
    const h2 = m ? Number(m[2]) : NaN;
    const ratio2 = w2 / h2;
    if (!m || w2 % 16 !== 0 || h2 % 16 !== 0 || ratio2 > 3 || ratio2 < 1 / 3 || w2 > MAX_EDGE || h2 > MAX_EDGE || w2 * h2 > MAX_PIXELS || w2 * h2 < MIN_PIXELS) {
      throw new McpError(
        ErrorCode.InvalidParams,
        `Invalid size '${opts.size}' for ${caps.displayName}. Rules: WIDTHxHEIGHT, both divisible by 16, aspect ratio between 1:3 and 3:1, max edge ${MAX_EDGE}px, total pixels ${MIN_PIXELS}\u2013${MAX_PIXELS}.`
      );
    }
    return opts.size;
  }
  ...
  if (ratio === void 0) {
    throw new McpError(
      ErrorCode.InvalidParams,
      `Unknown aspect ratio '${opts.aspectRatio}'. Supported: ${Object.keys(ASPECT_RATIOS).join(", ")}.`
    );
  }
...
function validateModelSpecificParams(modelId, params) {
  const capabilities = getModelCapabilities(modelId);
  if (params.background === "transparent" && !capabilities.supportsTransparentBackground) {
    throw new McpError(
      ErrorCode.InvalidParams,
      `Model ${capabilities.displayName} does not support transparent backgrounds. Use model 'gpt-image-1.5' for background: "transparent" (output is PNG with alpha).`
    );
  }
  if (params.forEditing && !capabilities.supportsEdits) {
    throw new McpError(
      ErrorCode.InvalidParams,
      `Model ${capabilities.displayName} is not available on the image-edits endpoint. Use model 'gpt-image-1.5' for editing and reference-image workflows.`
    );
  }
  if (params.referenceImageCount !== void 0 && params.referenceImageCount > capabilities.maxReferenceImages) {
    throw new McpError(
      ErrorCode.InvalidParams,
      `Too many reference images (${params.referenceImageCount}). Model ${capabilities.displayName} supports max ${capabilities.maxReferenceImages} input images.`
    );
  }
}
...
async function saveImageResult(result, session, lastImageState, overrideOutputDirectory) {
  const savedFiles = [];
  const imagesDir = overrideOutputDirectory ?? getImagesDirectory(session);
  await fs2.mkdir(imagesDir, { recursive: true, mode: 493 });
  for (const image of result.images) {
    ...
    const filePath = path3.join(imagesDir, fileName);
...
var SessionManager = class {
  ...
  // ─── Helper methods ───────────────────────────────────────────────────────
  generateSessionId() {
    const randomPart = randomUUID2().slice(0, 8);
  ...
  /**
   * Ensures an active session exists, creating a temporary one if needed.
   * This is the "forgiving default" behavior for LLM users who might forget to start a session.
   */
  async ensureActiveSession(options) {
    ...
    if (this.activeSessions.size >= MAX_SESSIONS) {
      throw new McpError(
        ...
        `Maximum number of sessions (${MAX_SESSIONS}) reached. Please end an existing session first using end_creative_session.`
      );
    }
...
async function handleStartCreativeSession(request, configMgr, sessionMgr) {
  ...
  if (outputDirectory) {
    ...
    try {
      await fs3.mkdir(resolvedOutputDir, { recursive: true, mode: 493 });
      const testFile = path4.join(resolvedOutputDir, ".test-write-access");
      await fs3.writeFile(testFile, "");
      await fs3.unlink(testFile);
    } catch (error) {
      throw new McpError(
        ErrorCode.InvalidParams,
        `Cannot write to output directory ${resolvedOutputDir}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  try {
...
\u26A1 OpenAI Image Settings:`;
      if (quality) responseText += `
  Quality: ${quality}`;
      if (background) responseText += `
  Background: ${background}`;
      if (size) responseText += `
  Size: ${size}`;
    }
...
\u{1F4A1} Use send_creative_message to generate images in this session.
\u{1F4A1} The session will expire after 30 minutes of inactivity.`;
    if (resolvedOutputDir) {
      responseText += `
\u{1F4A1} All images will be saved to your custom directory.`;
    }
    return {
...
async function handleEndCreativeSession(request, sessionMgr) {
  ...
  const duration = (/* @__PURE__ */ new Date()).getTime() - session.createdAt.getTime();
...
\u{1F4A1} Use start_creative_session to start a new session.`
      }
    ]
  };
}
async function handleGetSessionInfo(request, sessionMgr) {
  const { sessionId } = request.params.arguments;
  sessionMgr.cleanupExpiredSessions();
  if (sessionId) {
    ...
    const duration = (/* @__PURE__ */ new Date()).getTime() - session.createdAt.getTime();
    const durationMinutes = Math.round(duration / 6e4);
    const timeSinceActivity = (/* @__PURE__ */ new Date()).getTime() - session.lastActivityAt.getTime();
    const minutesSinceActivity = Math.round(timeSinceActivity / 6e4);
    let detailedInfo = `\u{1F4CA} Session Information
...
Aspect Ratio: ${session.config.aspectRatio || "default"}`;
    if (session.config.quality) detailedInfo += `
Quality: ${session.config.quality}`;
    if (session.config.background) detailedInfo += `
Background: ${session.config.background}`;
    if (session.config.size) detailedInfo += `
    ...
    if (sessionMgr.activeSessions.size === 0) {
      return {
        content: [
          {
            type: "text",
            text: "\u{1F4CA} No active sessions\n\n\u{1F4A1} Use start_creative_session to start a new session."
          }
        ]
      };
    }
    let statusText = `\u{1F4CA} Active Sessions (${sessionMgr.activeSessions.size}/${MAX_SESSIONS})

`;
    for (const [id, session] of sessionMgr.activeSessions.entries()) {
      const isCurrent = id === sessionMgr.currentSessionId;
      const duration = (/* @__PURE__ */ new Date()).getTime() - session.createdAt.getTime();
      const durationMin = Math.round(duration / 6e4);
...
`;
    ...
    statusText += `\u{1F4A1} Use get_session_info with sessionId for detailed information.`;
    return {
...
function toResult(response) {
  const images = (response.data ?? []).filter((d) => typeof d.b64_json === "string" && d.b64_json.length > 0).map((d) => ({ b64: d.b64_json }));
  return { images };
}
async function callGenerateImage(client, opts) {
  const response = await client.images.generate(
    buildRequestParams(opts)
  );
  return toResult(response);
}
async function callEditImage(client, opts) {
  const files = await Promise.all(
    opts.imagePaths.map(
      (p) => toFile(fs4.createReadStream(p), path5.basename(p), { type: getMimeType(p) })
    )
  );
...
function mapOpenAIError(error, context) {
  ...
  if (error instanceof OpenAI.APIError) {
    ...
    if (error.status === 401) {
      return new McpError(
        ErrorCode.InvalidRequest,
        `OpenAI API key rejected (401). Fix OPENAI_API_KEY or re-run configure_openai_token. [${context}]`
      );
    }
    if (error.status === 403 && /verif/i.test(message)) {
      return new McpError(
        ErrorCode.InvalidRequest,
        `Your OpenAI organization must complete Organization Verification before using GPT Image models. Visit platform.openai.com \u2192 Settings \u2192 Organization \u2192 Verification. [${context}]`
      );
    }
    if (/billing|insufficient.?quota|hard limit/i.test(message)) {
      return new McpError(
        ErrorCode.InvalidRequest,
        `OpenAI billing/quota limit reached: ${message} Add credits or raise the usage limit at platform.openai.com \u2192 Settings \u2192 Billing. [${context}]`
      );
    }
    if (error.status === 400 && /background/i.test(message)) {
      return new McpError(
        ErrorCode.InvalidParams,
        `This model rejected the background parameter (${message}). Transparent backgrounds require model 'gpt-image-1.5'. [${context}]`
      );
    }
    if (error.status === 400 && /(moderation|safety|content policy)/i.test(message)) {
      return new McpError(
        ErrorCode.InvalidParams,
        `Prompt or image blocked by OpenAI moderation: ${message}. Rephrase the prompt. [${context}]`
      );
    }
    return new McpError(
      ErrorCode.InternalError,
      `OpenAI API error (${error.status ?? "unknown"}): ${message} [${context}]`
    );
  }
  return new McpError(
    ErrorCode.InternalError,
...
// src/image-tools.ts
async function handleGenerateImage(request, configMgr, sessionMgr, lastImageState) {
  if (!configMgr.ensureConfigured()) {
    throw new McpError(ErrorCode.InvalidRequest, "OpenAI API token not configured. Use configure_openai_token first.");
  }
  const {
...
Aspect Ratio: ${aspectRatio} (size: ${resolvedSize})`;
    else if (resolvedSize !== "auto") statusText += `
Size: ${resolvedSize}`;
    if (quality) statusText += `
Quality: ${quality}`;
...
\u{1F4A1} Auto-session created (${sessionId}). Images tracked for organization.`;
    }
...
async function handleEditImage(request, configMgr, sessionMgr, lastImageState) {
  if (!configMgr.ensureConfigured()) {
    throw new McpError(ErrorCode.InvalidRequest, "OpenAI API token not configured. Use configure_openai_token first.");
  }
  const {
  ...
  if (!mainImageValidation.valid) {
    throw new McpError(
      ErrorCode.InvalidParams,
      `Main image error: ${mainImageValidation.error} (${imagePath})`
    );
  }
  try {
    const sessionId = await sessionMgr.ensureActiveSession({
...
Quality: ${quality}`;
    if (background) statusText += `
Background: ${background}`;
    if (validReferenceImages.length > 0) statusText += `
\u2705 Reference images used: ${validReferenceImages.length}`;
    if (skippedReferenceImages.length > 0) statusText += `
...
async function handleContinueEditing(request, configMgr, sessionMgr, lastImageState) {
  if (!configMgr.ensureConfigured()) {
    throw new McpError(ErrorCode.InvalidRequest, "OpenAI API token not configured. Use configure_openai_token first.");
  }
  ...
  try {
    await fs5.access(lastImageState.path);
  } catch {
    throw new McpError(
      ErrorCode.InvalidRequest,
      `Last image file not found at: ${lastImageState.path}. Please generate a new image first.`
    );
  }
...
async function handleGetLastImageInfo(lastImageState) {
  ...
  try {
    await fs5.access(lastImageState.path);
    const stats = await fs5.stat(lastImageState.path);
    return {
      content: [
        {
          type: "text",
          text: `\u{1F4F7} Last Image Information:
...
\u{1F4A1} The image file may have been moved or deleted. Please generate a new image.`
        }
      ]
    };
  }
}
async function handleSendCreativeMessage(request, configMgr, sessionMgr, lastImageState) {
  if (!configMgr.ensureConfigured()) {
    throw new McpError(ErrorCode.InvalidRequest, "OpenAI API token not configured. Use configure_openai_token first.");
  }
  ...
  if (!session) {
    ...
    throw new McpError(
      ErrorCode.InvalidRequest,
      `No active session found (${targetId}). Use start_creative_session first.`
    );
  }
  ...
  if (images && images.length > 0) {
    for (const imagePath of images) {
      ...
      if (!validation.valid) {
        throw new McpError(
          ErrorCode.InvalidParams,
          `Invalid image: ${validation.error} (${imagePath})`
        );
      }
  ...
  validateModelSpecificParams(cfg.model, {
    ...
    referenceImageCount: validatedImages.length > 0 ? validatedImages.length : void 0
  });
  try {
    const callOpts = {
      model: cfg.model,
    ...
    const result = validatedImages.length > 0 ? await callEditImage(configMgr.openai, { ...callOpts, imagePaths: validatedImages }) : await callGenerateImage(configMgr.openai, callOpts);
...
var PROMPT_TEMPLATES = {
  sprite_character: `# \u{1F3AE} Sprite Character Template

**Best for:** Character sprites with consistent poses for game development

## \u{1F3C6} Recommended Workflow: Hybrid Approach
...
## \u{1F4A1} Example (Base Character - Idle Pose)
...
## \u{1F4DD} Follow-up Prompts (With Image References!)
...
**Solution:** ALWAYS use \`images\` parameter in send_creative_message!`,
  game_environment: `# \u{1F3DE}\uFE0F Game Environment Template
...
## \u{1F4A1} Example: Side-Scrolling Forest
...
- Quality: high (environment detail warrants it)`,
  game_asset: `# \u{1F3B2} Game Asset Template
...
- Quality: medium
- For isolated sprites: model gpt-image-1.5 + background: "transparent" (skips manual background removal).`,
  character_concept: `# \u{1F464} Character Concept Template

**Best for:** Initial character establishment with detailed specifications

## \u{1F3C6} Recommended Workflow
...
- SAVE PATH: Use for character_variations next!`,
  character_variations: `# \u{1F3AD} Character Variations Template

**Best for:** Expressions, poses, outfits while maintaining consistency

## \u{1F3C6} CRITICAL: Must Use Hybrid Workflow
...
## \u{1F4DD} Prompt Template (For Variations)
...
## \u{1F4A1} Examples (With Image References!)
...
- Proportion drift
- Style inconsistency`,
  character_portrait: `# \u{1F5BC}\uFE0F Character Portrait Template
...
## \u{1F4A1} Example: RPG Dialogue Portrait
...
- Quality: high (portrait detail warrants it)`,
  mobile_mockup: `# \u{1F4F1} Mobile UI Mockup Template
...
## \u{1F4A1} Example: Fitness Tracker App
...
- Single image output: All 4 screens in one composition`,
  icon_set: `# \u{1F3A8} Icon Set Template

**Best for:** Consistent icon families for applications

## \u{1F3C6} Recommended Workflow: Hybrid for Consistency
...
## \u{1F4DD} Prompt Template (First Icon)
...
## \u{1F4A1} Example: Navigation Icon Set
...
- For isolated sprites: model gpt-image-1.5 + background: "transparent" (skips manual background removal).`,
  ui_component: `# \u{1F9E9} UI Component Template
...
## \u{1F4A1} Example: Primary CTA Button
...
- Quality: medium`,
  web_section: `# \u{1F310} Web Section Template
...
## \u{1F4A1} Example: SaaS Hero Section
...
See the \`openai-prompts\` skill \xA7 "The Identity Guide Pattern" for the full 5-line guide structure and rationale.
...
// src/tool-schemas.ts
var ASPECT_RATIO_ENUM = ["1:1", "2:3", "3:2", "3:4", "4:3", "4:5", "5:4", "9:16", "16:9", "21:9"];
var MODEL_ENUM = ["gpt-image-2", "gpt-image-1.5"];
var MODEL_PROP = {
  type: "string",
  enum: [...MODEL_ENUM],
  description: "GPT Image model (default: gpt-image-2). gpt-image-2: best quality, arbitrary sizes up to 3840px, NO transparent background. gpt-image-1.5: supports background:'transparent' (alpha-cut sprites), preset sizes only."
};
var MODEL_PROP_SESSION = {
  type: "string",
  enum: [...MODEL_ENUM],
  description: "GPT Image model to use for this session (default: gpt-image-2). gpt-image-2: best quality, arbitrary sizes up to 3840px, NO transparent background. gpt-image-1.5: supports background:'transparent' (alpha-cut sprites), preset sizes only."
};
var QUALITY_PROP = {
  type: "string",
  enum: ["low", "medium", "high", "auto"],
  description: "Rendering quality. Default 'auto' (model decides). 'low' is fastest/cheapest for drafts; 'high' for finals."
};
var QUALITY_PROP_SESSION = {
  type: "string",
  enum: ["low", "medium", "high", "auto"],
  description: "Session default rendering quality. Default 'auto' (model decides). 'low' is fastest/cheapest for drafts; 'high' for finals."
};
var BACKGROUND_PROP = {
  type: "string",
  enum: ["transparent", "opaque", "auto"],
  description: "Background mode. 'transparent' produces PNG with alpha \u2014 REQUIRES model gpt-image-1.5; gpt-image-2 rejects it. Ideal for game sprites (no post-cut needed). Default 'auto'."
};
var BACKGROUND_PROP_SESSION = {
  type: "string",
  enum: ["transparent", "opaque", "auto"],
  description: "Session default background mode. 'transparent' produces PNG with alpha \u2014 REQUIRES model gpt-image-1.5; gpt-image-2 rejects it. Ideal for game sprites (no post-cut needed). Default 'auto'."
};
var SIZE_PROP = {
  type: "string",
...
var ASPECT_RATIO_PROP = {
  ...
  description: "Aspect ratio for the generated image. Options: 1:1 (square), 2:3 (portrait), 3:2 (landscape), 3:4 (portrait), 4:3 (landscape), 4:5 (portrait), 5:4 (landscape), 9:16 (vertical/mobile), 16:9 (widescreen), 21:9 (ultrawide). Extreme ratios beyond 3:1 are not supported by GPT Image models. Default: 1:1"
};
var ASPECT_RATIO_PROP_EDIT = {
  type: "string",
  enum: [...ASPECT_RATIO_ENUM],
  description: "Aspect ratio for the edited image. Options: 1:1 (square), 2:3 (portrait), 3:2 (landscape), 3:4 (portrait), 4:3 (landscape), 4:5 (portrait), 5:4 (landscape), 9:16 (vertical/mobile), 16:9 (widescreen), 21:9 (ultrawide). Extreme ratios beyond 3:1 are not supported by GPT Image models. Default: maintains original aspect ratio"
};
var ASPECT_RATIO_PROP_SESSION = {
  type: "string",
  enum: [...ASPECT_RATIO_ENUM],
  description: "Session default aspect ratio for images. Options: 1:1 (square), 2:3 (portrait), 3:2 (landscape), 3:4 (portrait), 4:3 (landscape), 4:5 (portrait), 5:4 (landscape), 9:16 (vertical/mobile), 16:9 (widescreen), 21:9 (ultrawide). Extreme ratios beyond 3:1 are not supported by GPT Image models."
};
var TOOL_SCHEMAS = [
  {
...
var OpenAICreativeMCP = class {
  ...
  async run() {
    const logFile = path6.join(os2.homedir(), "openai-mcp-debug.log");
    const log = (message) => {
      const timestamp = (/* @__PURE__ */ new Date()).toISOString();
      const logMessage = `[${timestamp}] ${message}
`;
      console.error(message);
      fs6.appendFile(logFile, logMessage).catch(() => {
      });
    };
    try {
      log("[OpenAI Creative Plugin] \u{1F504} Starting server...");
      ...
      log("[OpenAI Creative Plugin] \u{1F50C} Connecting transport...");
      await this.server.connect(transport);
      log("[OpenAI Creative Plugin] \u{1F680} Server started and ready for connections");
    } catch (error) {
...
var server = new OpenAICreativeMCP();
server.run().catch((error) => {
  console.error("[OpenAI Creative Plugin] \u274C Fatal error:", error);
  process.exit(1);
});
